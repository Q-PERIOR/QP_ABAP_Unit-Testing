...
const packageToRun = "Z2607_AZURE_UNIT_TEST"; // ABAP Unit-Testing Package in S4H
...
function readXml() {
    ...
    const xmlRunAbapUnit = xmlRunAbapUnitBuffer.toString('utf8').replace("{{package}}", packageToRun); // insert the defined ABAP Test-Package in string of xml-Data
...
function getCSRFTokenOptions() {
    return {
        method: "GET",
        rejectUnauthorized: false, // sonst Fehlermeldung - GGF. WIEDER ENTFERNEN !!!!
        url: 'https://s4htcsaptui.westeurope.cloudapp.azure.com:44301/sap/opu/odata/SAP/adt_srv',
        simple: false,
        resolveWithFullResponse: true,
        auth: {
            user: "4112",
            password: "mulitzka@SAP22",
        },
        headers: {
            'X-CSRF-Token': 'fetch'
        }
...
function getRunAbapUnitTestOptions(xmlRunAbapUnit, xCSRFToken) {
    return {
        method: 'POST',
        rejectUnauthorized: false, // sonst Fehlermeldung - GGF. WIEDER ENTFERNEN !!!!
        url: 'https://s4htcsaptui.westeurope.cloudapp.azure.com:44301/sap/bc/adt/abapunit/testruns',
        auth: {
            user: "4112",
            password: "mulitzka@SAP22",
        },
...
function main() {
    ...
    runAbapUnitTestPromise.then(function (parsedBody) {
        ...
        fs.writeFileSync("output.xml", outXmlString); // write xml-File with ABAP Unit-Testing results
